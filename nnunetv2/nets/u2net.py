...
def get_u2netp_from_plans(
    ...
    model = U2NETP(in_ch=num_input_channels, out_ch=label_manager.num_segmentation_heads,
...
def get_u2net_from_plans(
    ...
    model = U2NET(in_ch=num_input_channels, out_ch=label_manager.num_segmentation_heads,
                  deep_supervision=deep_supervision)
    model.apply(InitWeights_He(1e-2))
    model.apply(init_last_bn_before_add_to_0)
...
if __name__ == '__main__':
    model = U2NET().to("cpu")
    output = model(torch.rand(size=(1, 3, 224, 224)).to("cpu"))
    print(output.shape)
